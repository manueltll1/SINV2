<template>
  <div class="grid container">
    ...
    <div class="formgroup-inline filters col-12 lg:col-12 xl:col-12">
      <!-- Fecha Inicio -->
      <div class="col-6 lg:col-2 xl:col-2 p-card-element">
        <label for="fechaIni" class="p-sr-only">Fecha Inicio</label>
        <input class="p-inputtext p-component" id="fechaIni" type="date" v-model="filters.ini" placeholder="Fecha Inicio">
      </div>
      <!-- Fecha Fin -->
      <div class="col-6 lg:col-2 xl:col-2 p-card-element">
          <label for="fechaFin" class="p-sr-only">Fecha Fin</label>
          <input class="p-inputtext p-component" id="fechaFin" type="date" v-model="filters.fin" placeholder="Fecha Fin">
      </div>
      <!-- Ramo -->
      <div class="col-6 lg:col-1 xl:col-1 p-card-element">
        <Select   v-if="ramosOptions && ramosOptions.length" v-model="filters.ramo" :options="ramosOptions" filter optionLabel="label" placeholder="RAMOS" class="w-full">
      ...
      <div class="col-12 lg:col-2 xl:col-2 p-card-element">
    ...
    <div class="col-12 lg:col-6 xl:col-2">
            ...
            <div class="flex">
              ...
              <div class="text-900 font-medium text-xl ml-3">{{ c1 }}</div>
					...
					<div class="flex align-items-center justify-content-center bg-yellow-100 border-round" style="width:2.5rem;height:2.5rem">
						<i class="text-black-500 text-xl">{{ mp > 0 ? '😎' : '🙇🏻‍♂️' }}</i>
					</div>
				</div>
				<div class="flex">
    ...
    <div class="col-12 lg:col-6 xl:col-3">
            ...
            <div class="flex">
              ...
              <div class="font-medium text-xl ml-3" :class="{'text-green-500': pn24 >= pn23, 'text-red-500': pn24 < pn23}">{{ formatCurrency(mpn) }}</div>
					...
					<div class="flex align-items-center justify-content-center bg-green-100 border-round" style="width:2.5rem;height:2.5rem">
						<i class="text-black-500 text-xl">{{ cpn > 0 ? '😎' : '🙇🏻‍♂️' }}</i>
					</div>
				</div>
				<div class="flex">
    ...
    <div class="col-12 lg:col-7 xl:col-7">
      <div class="p-card pb-0 pt-0">
        <!-- <h5>Clientes</h5> -->
        <TabView style="font-size:0.8rem">
            <TabPanel header="Nuevos">
              <DataTable v-if="tableCrecimiento.length" tableStyle="min-width: 50rem" style="font-size:10px" :value="clientesGanados" :rows="3" :paginator="true" responsiveLayout="scroll">
                <Column field="cliente" header="Nombre"></Column>
              </DataTable>
            </TabPanel>
            <TabPanel header="Perdidos">
              <DataTable :size="small" tableStyle="min-width: 50rem" style="font-size:10px" :value="clientesPerdidos" :rows="3" :paginator="true" responsiveLayout="scroll">
                <Column field="cliente" header="Nombre"></Column>
              </DataTable>
            </TabPanel>
        </TabView>
  ...
  export default {
    ...
    data() {
      return {
        ...
        filtersEmitidasinv: { // Inicializa el objeto con los filtros necesarios
          global: { value: '', matchMode: 'contains' },
          no_poliza: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          fec_vig_de: { operator: 'and', constraints: [{ value: null, matchMode: 'dateIs' }] },
          pn_tc: { operator: 'and', constraints: [{ value: null, matchMode: 'equals' }] },
          cliente: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          concepto: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          vendedor: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          grupo: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          ramo: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          aseguradora: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          estatus_mov: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          tipo_movimiento: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
          fec_cancelacion: { operator: 'and', constraints: [{ value: null, matchMode: 'dateIs' }] }
        },
        tableCrecimiento: [],
    ...
    methods: {
      ...
      async fetchData(filters = {}) {
        try {
            const response = await http().get("/dashboard-data", { params: filters });
      ...
      initFilters() {
          ...
          this.filtersEmitidasinv = {
              global: { value: `/${month}/`, matchMode: 'contains' },
              no_poliza: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
              fec_vig_de: { operator: 'and', constraints: [{ value: null, matchMode: 'dateIs' }] },
              pn_tc: { operator: 'and', constraints: [{ value: null, matchMode: 'equals' }] },
              cliente: { operator: 'and', constraints: [{ value: null, matchMode: 'contains' }] },
      ...
      setInitialDates() {
        ...
        this.filters.ini = this.formatDateF(firstDayOfMonth);
        this.filters.fin = this.formatDateF(lastDayOfMonth);
      },
      formatDateF(date) {
